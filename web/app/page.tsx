...
const DETECTORS: Array<{ type: string; re: RegExp; mask: (v: string) => string }> = [
  {
    ...
    mask: (v) => "XXXX XXXX " + v.replace(/\s/g, "").slice(-4),
  },
  {
    type: "PAN",
    re: /\b[A-Z]{5}[0-9]{4}[A-Z]\b/g,
    mask: (v) => v.slice(0, 2) + "XXX" + v.slice(5),
  },
  {
    type: "IFSC",
  ...
  {
    type: "UPI",
    re: /\b[a-zA-Z0-9._-]{2,}@[a-zA-Z]{2,}\b/g,
    mask: (v) => {
  ...
  },
  {
    type: "PHONE",
    re: /\b[6-9]\d{9}\b/g,
    mask: (v) => v.slice(0, 2) + "XXXX" + v.slice(-4),
  },
];
  ...
  DETECTORS.forEach((d) => {
    ...
    while ((m = re.exec(text)) !== null) {
      found.push({
        ...
        masked: d.mask(m[0]),
        pos: `chars ${m.index}–${m.index + m[0].length}`,
      });
    }
  });
...
function Annotated({ text, entities }: { text: string; entities: Entity[] }) {
  ...
  entities.forEach((e, i) => {
    ...
    nodes.push(
      <span
        key={i}
        style={{ borderBottom: `1px solid ${ACCENT}`, background: ACCENT_BG, padding: "0 1px" }}
      >
...
function HeroTerminal({ step }: { step: number }) {
  ...
  const lines: ReactNode[] = [
    ...
    <div key={4} style={{ lineHeight: 1.8 }}>
      ...
      <br />
      {"   UPI "}<Hl>ravi@okhdfc</Hl>{`"`}
    </div>,
    <div key={5} style={{ height: 10 }} />,
    <div key={6}>
      <span style={{ color: ACCENT }}>⚠ 3 entities redacted</span>{" · 0 bytes to model"}
    </div>,
  ];
...
function useHeroCanvas(ref: React.RefObject<HTMLCanvasElement | null>) {
  useEffect(() => {
    ...
    interface Pkt {
      x: number; y: number; lane: number; speed: number;
      switching: boolean; fromLane: number; toLane: number;
      sx: number; ex: number; hist: Array<{ x: number; y: number }>;
    }
    let packets: Pkt[] = [];
    const SWITCH_LEN = 72;
    ...
    const draw = () => {
      ...
      ctx.strokeStyle = "rgba(13,13,11,0.07)";
      ctx.lineWidth = 1;
      ctx.beginPath();
      ...
      junctions.forEach((jx) => {
        ctx.strokeStyle = "rgba(13,13,11,0.045)";
        ctx.beginPath(); ctx.moveTo(jx + 0.5, topY - 9); ctx.lineTo(jx + 0.5, botY + 9); ctx.stroke();
        ctx.fillStyle = "rgba(13,13,11,0.15)";
        lanes.forEach((y) => ctx.fillRect(jx - 1, y - 1, 2, 2));
      });
        ...
        if (!p.switching) {
          ...
          junctions.forEach((jx) => {
            if (p.x >= jx && p.x - p.speed < jx && Math.random() < 0.5) {
              ...
              let target = p.lane + dir;
              if (target < 0) target = p.lane + 1;
              if (target >= lanes.length) target = p.lane - 1;
              if (target !== p.lane) {
                p.switching = true; p.fromLane = p.lane; p.toLane = target;
                p.sx = jx; p.ex = jx + SWITCH_LEN;
              }
            }
          });
        } else {
          const tt = Math.min(1, (p.x - p.sx) / (p.ex - p.sx));
          const e = tt * tt * (3 - 2 * tt);
          p.y = lanes[p.fromLane] + (lanes[p.toLane] - lanes[p.fromLane]) * e;
          ctx.strokeStyle = "rgba(42,111,219,0.22)";
          ctx.beginPath(); ctx.moveTo(p.sx, lanes[p.fromLane]); ctx.lineTo(p.ex, lanes[p.toLane]); ctx.stroke();
          ctx.fillStyle = "rgba(42,111,219,0.5)";
          ctx.fillRect(p.sx - 2, lanes[p.fromLane] - 2, 4, 4);
          ctx.fillRect(p.ex - 2, lanes[p.toLane] - 2, 4, 4);
          if (tt >= 1) { p.switching = false; p.lane = p.toLane; p.y = lanes[p.lane]; }
        }
        if (p.x > w + 30) {
          p.x = -30; p.lane = Math.floor(Math.random() * lanes.length);
          p.switching = false; p.y = lanes[p.lane]; p.hist = [];
        }
        p.hist.push({ x: p.x, y: p.y });
        if (p.hist.length > 10) p.hist.shift();
        p.hist.forEach((h, i) => {
          ctx.fillStyle = `rgba(42,111,219,${0.24 * (i / p.hist.length)})`;
          ctx.fillRect(h.x - 1, h.y - 1, 2, 2);
        });
        ctx.fillStyle = "rgba(42,111,219,0.78)";
  ...
  return (
    <>
      {lines.map((line, i) => (
        <div key={i}>
          {line.map((part, j) => (
            <span key={j} style={part.c ? { color: part.c } : undefined}>{part.t}</span>
          ))}
        </div>
      ))}
    </>
    ...
    <div style={{ background: "#FAFAF8", minHeight: "100vh", fontFamily: "'Satoshi', system-ui, sans-serif" }}>
      ...
      <nav style={{ position: "sticky", top: 0, zIndex: 50, background: "rgba(250,250,248,0.88)", backdropFilter: "saturate(180%) blur(8px)", borderBottom: "1px solid #E8E8E4" }}>
        <div style={{ maxWidth: 1180, margin: "0 auto", padding: "0 40px", height: 64, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div style={{ display: "flex", gap: 36, fontFamily: MONO, fontSize: 13, color: "#71716B" }}>
          ...
          <a href="#demo" style={{ fontFamily: MONO, fontSize: 13, color: ACCENT, whiteSpace: "nowrap" }}>Request access&nbsp;→</a>
        </div>
      </nav>
      ...
      <section style={{ position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", zIndex: 1, maxWidth: 1180, margin: "0 auto", padding: "88px 40px 72px" }}>
          <div style={{ display: "grid", gridTemplateColumns: "55fr 45fr", gap: 56, alignItems: "start" }}>
            <div>
              ...
              <h1 style={{ fontSize: 58, lineHeight: 1.04, fontWeight: 700, letterSpacing: "-0.035em", margin: "0 0 28px", color: "#0D0D0B" }}>
                Your AI stack.<br />Your data.<br /><span style={{ color: ACCENT }}>Always.</span>
              </h1>
              <p style={{ fontSize: 17, lineHeight: 1.65, color: "#71716B", margin: "0 0 36px", maxWidth: 440 }}>
            ...
            <div style={{ border: "1px solid #E8E8E4", background: "#FFFFFF", borderRadius: 6, overflow: "hidden" }}>
              ...
              <div style={{ padding: "20px 18px", minHeight: 236 }}>
      ...
      <div style={{ maxWidth: 1180, margin: "0 auto" }}><div style={{ borderTop: "1px solid #E8E8E4" }} /></div>

      {/* ── LOGO STRIP ── */}
      <section style={{ maxWidth: 1180, margin: "0 auto", padding: "28px 40px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 48, flexWrap: "wrap" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 44, flexWrap: "wrap", flex: 1, justifyContent: "space-between" }}>
            {["Razorbank", "Meridian", "Kosh", "Northwind", "Tessellate", "Anvaya", "Settl"].map((name) => (
              <span key={name} style={{ fontSize: 18, fontWeight: 700, letterSpacing: "-0.02em", color: "#B0B0A8" }}>{name}</span>
            ))}
          </div>
        </div>
      </section>
      ...
      <section id="process" style={{ background: "#FFFFFF", borderTop: "1px solid #E8E8E4", borderBottom: "1px solid #E8E8E4" }}>
        <div style={{ maxWidth: 1180, margin: "0 auto", padding: "88px 40px" }}>
          ...
          <h2 style={{ fontSize: 38, fontWeight: 700, letterSpacing: "-0.03em", margin: "0 0 56px", color: "#0D0D0B", maxWidth: 640, lineHeight: 1.08 }}>
            One line of code. Five layers of <span style={{ color: ACCENT }}>protection.</span>
          </h2>

          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 48 }}>
            {["[01]", "[02]", "[03]"].map((n) => (
              <div key={n} style={{ fontFamily: MONO, fontSize: 13, color: "#71716B" }}>{n}</div>
            ))}
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 48, marginTop: 10 }}>
            {["Wrap", "Shield", "Route"].map((v) => (
              <div key={v} style={{ fontSize: 18, fontWeight: 700, letterSpacing: "-0.02em" }}>{v}</div>
            ))}
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 48 }}>
            <p style={{ fontSize: 15, lineHeight: 1.6, color: "#71716B", margin: 0 }}>
              Wrap any client — <span style={{ fontFamily: CODE_FONT, fontSize: 13, color: "#0D0D0B" }}>svitch.wrap(openai.client)</span>. Context and tools carry across every model.
            </p>
            <p style={{ fontSize: 15, lineHeight: 1.6, color: "#71716B", margin: 0 }}>
      ...
      <section id="demo" style={{ background: "#F4F7F5", borderBottom: "1px solid #E8E8E4" }}>
        <div style={{ maxWidth: 1180, margin: "0 auto", padding: "88px 40px" }}>
          <div style={{ display: "grid", gridTemplateColumns: "38fr 62fr", gap: 56, alignItems: "start" }}>
            <div style={{ position: "sticky", top: 96 }}>
              <div style={{ fontFamily: MONO, fontSize: 11, letterSpacing: "0.1em", color: "#71716B", marginBottom: 18 }}>SVITCH SHIELD · LIVE DEMO</div>
              <h2 style={{ fontSize: 32, fontWeight: 700, letterSpacing: "-0.03em", lineHeight: 1.1, margin: "0 0 18px" }}>
                Paste any text. Watch PII <span style={{ color: ACCENT }}>disappear.</span>
              </h2>
              <p style={{ fontSize: 15, lineHeight: 1.65, color: "#71716B", margin: 0, maxWidth: 300 }}>
                Runs entirely in your browser here — the same engine runs inline, server-side, before any model sees a single token.
              </p>
            </div>
            ...
            <div style={{ background: "#FFFFFF", border: "1px solid #E8E8E4", borderRadius: 6, padding: 20 }}>
              ...
              <textarea
                ...
                spellCheck={false}
                style={{ width: "100%", height: 108, resize: "vertical", border: "1px solid #E8E8E4", borderRadius: 4, background: "#F4F3F0", padding: 13, fontSize: 13, lineHeight: 1.6, color: "#0D0D0B", outline: "none", fontFamily: CODE_FONT }}
              />

              <div style={{ display: "flex", alignItems: "center", gap: 16, marginTop: 14 }}>
                <button
                  onClick={onDetect}
                  style={{ fontFamily: "'Satoshi', sans-serif", fontSize: 14, fontWeight: 500, color: "#0D0D0B", background: "transparent", border: "1px solid #0D0D0B", borderRadius: 4, padding: "9px 18px", cursor: "pointer" }}
                >
                  Detect &amp; Redact
                </button>
                <button
                  onClick={onClear}
                  style={{ fontFamily: MONO, fontSize: 12, color: "#71716B", background: "transparent", border: "none", cursor: "pointer" }}
                >
                  clear
                </button>
              </div>
                    ...
                    <div style={{ marginTop: 18, border: "1px solid #E8E8E4", borderRadius: 4, overflow: "hidden" }}>
                      <div style={{ display: "grid", gridTemplateColumns: "1.1fr 1.6fr 1.1fr", background: "#F4F3F0", borderBottom: "1px solid #E8E8E4" }}>
                        {["Entity type", "Value", "Position"].map((h, i) => (
                          <div key={h} style={{ padding: "9px 14px", fontSize: 12, fontWeight: 500, color: "#71716B", borderRight: i < 2 ? "1px solid #E8E8E4" : undefined }}>
                            {h}
                          </div>
                        ))}
                      </div>
                      {results.map((row, i) => (
                        <div key={i} style={{ display: "grid", gridTemplateColumns: "1.1fr 1.6fr 1.1fr", borderTop: "1px solid #E8E8E4" }}>
                          ...
                          <div style={{ padding: "9px 14px", fontFamily: MONO, fontSize: 12, color: "#0D0D0B", borderRight: "1px solid #E8E8E4" }}>{row.masked}</div>
      ...
      <section id="code" style={{ background: "#FFFFFF", borderBottom: "1px solid #E8E8E4" }}>
        <div style={{ maxWidth: 1180, margin: "0 auto", padding: "88px 40px" }}>
          <div style={{ display: "grid", gridTemplateColumns: "42fr 58fr", gap: 56, alignItems: "center" }}>
            <div>
              <div style={{ fontFamily: MONO, fontSize: 11, letterSpacing: "0.12em", color: "#71716B", marginBottom: 18 }}>THE SDK</div>
              <h2 style={{ fontSize: 38, fontWeight: 700, letterSpacing: "-0.03em", lineHeight: 1.08, margin: "0 0 20px" }}>
                Three lines.<br />Zero <span style={{ color: ACCENT }}>friction.</span>
              </h2>
              <p style={{ fontSize: 16, lineHeight: 1.65, color: "#71716B", margin: "0 0 18px", maxWidth: 380 }}>
            ...
            <div style={{ position: "relative", background: "#F4F3F0", border: "1px solid #E8E8E4", borderRadius: 4 }}>
              <button
                onClick={copyCode}
                style={{ position: "absolute", top: 12, right: 12, fontFamily: MONO, fontSize: 11, color: "#71716B", background: "#FFFFFF", border: "1px solid #E8E8E4", borderRadius: 3, padding: "5px 10px", cursor: "pointer" }}
              >
                {copied ? "copied" : "copy"}
              </button>
      ...
      <section style={{ background: "#FAFAF8" }}>
        <div style={{ maxWidth: 1180, margin: "0 auto", padding: "88px 40px 40px" }}>
          <div style={{ fontFamily: MONO, fontSize: 11, letterSpacing: "0.12em", color: "#71716B" }}>WHAT&apos;S INSIDE SVITCH</div>
        </div>

        {/* SHIELD */}
        <div style={{ maxWidth: 1180, margin: "0 auto", padding: "40px 40px", borderTop: "1px solid #E8E8E4" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 56, alignItems: "center" }}>
            <div>
              ...
              <h3 style={{ fontSize: 24, fontWeight: 700, letterSpacing: "-0.02em", margin: "0 0 12px" }}>PII redaction before the prompt</h3>
              <p style={{ fontSize: 15, lineHeight: 1.65, color: "#71716B", margin: 0, maxWidth: 420 }}>
        ...
        <div style={{ maxWidth: 1180, margin: "0 auto", padding: "40px 40px", borderTop: "1px solid #E8E8E4" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 56, alignItems: "center" }}>
            ...
            <div>
              ...
              <h3 style={{ fontSize: 24, fontWeight: 700, letterSpacing: "-0.02em", margin: "0 0 12px" }}>The right model for every query</h3>
              <p style={{ fontSize: 15, lineHeight: 1.65, color: "#71716B", margin: 0, maxWidth: 420 }}>
        ...
        <div style={{ maxWidth: 1180, margin: "0 auto", padding: "40px 40px", borderTop: "1px solid #E8E8E4" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 56, alignItems: "center" }}>
            <div>
              ...
              <h3 style={{ fontSize: 24, fontWeight: 700, letterSpacing: "-0.02em", margin: "0 0 12px" }}>A tamper-proof record of every decision</h3>
              <p style={{ fontSize: 15, lineHeight: 1.65, color: "#71716B", margin: 0, maxWidth: 420 }}>
            ...
            <div style={{ border: "1px solid #E8E8E4", background: "#FFFFFF", borderRadius: 6, padding: 18, fontFamily: CODE_FONT, fontSize: 12, color: "#0D0D0B" }}>
              ...
              <div style={{ marginTop: 12, fontSize: 11, color: "#71716B" }}>
                prev_hash verified · <span style={{ color: ACCENT }}>chain intact</span>
              </div>
            </div>
          </div>
        </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 56, alignItems: "center" }}>
            <div style={{ border: "1px solid #E8E8E4", background: "#FFFFFF", borderRadius: 6, padding: 18, fontFamily: CODE_FONT, fontSize: 12, color: "#0D0D0B" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                ...
                <span style={{ flex: 1, borderTop: `1px dashed ${ACCENT}`, margin: "0 8px" }} />
              ...
              <div style={{ marginTop: 16, fontSize: 11, color: "#71716B" }}>
                model weights resident · <span style={{ color: "#0D0D0B" }}>no outbound route</span>
              </div>
            </div>
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
              ...
              <h3 style={{ fontSize: 24, fontWeight: 700, letterSpacing: "-0.02em", margin: "0 0 12px" }}>Inference that never leaves your perimeter</h3>
              <p style={{ fontSize: 15, lineHeight: 1.65, color: "#71716B", margin: 0, maxWidth: 420 }}>
        ...
        {/* LEDGER */}
        <div style={{ maxWidth: 1180, margin: "0 auto", padding: "40px 40px", borderTop: "1px solid #E8E8E4", borderBottom: "1px solid #E8E8E4" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 56, alignItems: "center" }}>
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
              ...
              <h3 style={{ fontSize: 24, fontWeight: 700, letterSpacing: "-0.02em", margin: "0 0 12px" }}>Consent, recorded on a shared ledger</h3>
              <p style={{ fontSize: 15, lineHeight: 1.65, color: "#71716B", margin: 0, maxWidth: 420 }}>
            ...
            <div style={{ border: "1px solid #E8E8E4", background: "#FFFFFF", borderRadius: 6, padding: 18, fontFamily: CODE_FONT, fontSize: 12, color: "#0D0D0B" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {[
                  { id: "subject_8821", status: "granted", live: true },
                  { id: "subject_8822", status: "granted", live: true },
                  { id: "subject_8823", status: "withdrawn", live: false },
                ].map((row) => (
      ...
      <section style={{ background: "#0D0D0B", color: "#FFFFFF" }}>
        <div style={{ maxWidth: 1180, margin: "0 auto", padding: "84px 40px" }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 48 }}>
            {[
              ...
              <div key={stat}>
                <div style={{ fontFamily: DISPLAY, fontSize: 56, fontWeight: 700, letterSpacing: "-0.03em", lineHeight: 1, marginBottom: 18 }}>{stat}</div>
      ...
      <footer style={{ background: "#FAFAF8", borderTop: "1px solid #E8E8E4" }}>
        <div style={{ maxWidth: 1180, margin: "0 auto", padding: "48px 40px", display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 32, alignItems: "start" }}>
          <div>
          ...
          <div style={{ display: "flex", gap: 28, fontFamily: MONO, fontSize: 13, color: "#71716B", justifyContent: "center" }}>
            <a href="#process" style={{ color: "#71716B" }}>Products</a>
            <a href="#demo" style={{ color: "#71716B" }}>Docs</a>
            <a href="https://github.com/koushiknarendra/svitch" target="_blank" style={{ color: "#71716B" }}>GitHub</a>
        ...
        {/* Oversized wordmark */}
        <div style={{ maxWidth: 1180, margin: "0 auto", padding: "0 40px", overflow: "hidden" }}>
          <div
            ref={wordmarkRef}
